...
class GenarationInput(BaseModel):
    ...
    input_prompt: str = Field(
        ..., example="This ship is taking me far away", title="Input prompt"
    )
    generate_len: int = Field(..., example=500, title="Length of generated text")
...
class GenerationResult(BaseModel):
    ...
    generated_prompt = Field(
        ...,
        example=f"This ship is taking me far away" f"Far away from the memories",
...
class GenerationResponse(BaseModel):
    ...
    error: bool = Field(..., example=False, title="Is there error or not")
